...
const tiers = [
  ...
  {
    name: "Sakura",
    cls: "from-[#ffd6e5] to-[#f7a8c4] text-white shadow-pop",
  },
  {
    name: "Platinum",
    cls: "from-white to-[#ffe6f0] text-pink-ink ring-2 ring-pink-soft",
  },
];
...
const community = [
  {
    ...
    desc: "Public, private, community and scheduled event rooms — with raise-hand, speaker queues and host controls.",
  },
  {
    icon: "globe",
    title: "Live events with speakers",
    desc: "Join voice events with native speakers and guests. RSVP and add them straight to your calendar.",
  },
  {
    icon: "shield",
    title: "Safe & moderated",
    desc: "Report, block, mute and clear moderation roles keep the community kind and focused on learning.",
  },
];
    ...
    <section className="relative py-20 md:py-28">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <SectionHeading
          ...
          subtitle="Streaks, achievements and a friendly community turn steady practice into something you genuinely look forward to."
        />
          ...
          <div className="mt-6 flex flex-wrap items-end justify-center gap-4">
            {tiers.map((t, i) => (
              <div key={t.name} className="flex flex-col items-center gap-2">
                <span
                  className={`grid h-16 w-16 place-items-center rounded-2xl bg-gradient-to-br font-display text-xl font-extrabold shadow-soft ${t.cls}`}
                  style={{ transform: `translateY(${(2 - Math.abs(2 - i)) * -6}px)` }}
                >
                  ★
                </span>
                <span className="text-xs font-bold text-ink-soft">{t.name}</span>
              </div>
            ))}
          </div>
        </Reveal>
