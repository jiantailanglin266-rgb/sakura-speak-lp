      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <SectionHeading
          ...
          title={
            <>
              {t.pricing.title}
              <span className="text-gradient">{t.pricing.accent}</span>
            </>
          }
        ...
        <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {plans.map((p, i) => (
            ...
            >
              ...
              <h3 className={`font-display text-lg font-bold ${p.featured ? "text-white" : "text-ink"}`}>
                {t.pricing.planNames[i]}
              </h3>

              <div className="mt-3 flex items-end gap-1">
                <span className="font-display text-4xl font-extrabold">{p.price}</span>
              </div>
              <p className={`text-sm ${p.featured ? "text-white/80" : "text-ink-mute"}`}>
                {t.pricing.periods[i]}
              </p>
              {p.perMonth && (
                <p className={`mt-1 text-xs font-bold ${p.featured ? "text-white" : "text-pink-deep"}`}>
                  {p.perMonth}
                </p>
              )}

              <p className={`mt-4 flex-1 text-sm leading-relaxed ${p.featured ? "text-white/85" : "text-ink-soft"}`}>
                {t.pricing.planNotes[i]}
              </p>
              ...
              >
                {p.name === "Lifetime" ? t.pricing.ctaLifetime : t.pricing.ctaTrial}
              </CTAButton>
            </Reveal>
          ))}
        </div>
        ...
        <Reveal className="mx-auto mt-10 max-w-3xl">
          <ul className="grid gap-2.5 sm:grid-cols-3">
            {t.pricing.notes.map((n) => (
              ...
              >
                <span className="mt-0.5 text-pink-deep" aria-hidden>✓</span>
                {n}
              </li>
            ))}
          </ul>
          <p className="mt-6 text-center text-xs text-ink-mute">{t.pricing.secure}</p>
        </Reveal>
      </div>
    </section>
  );
